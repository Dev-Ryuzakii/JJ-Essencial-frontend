...
```typescript
// ✅ CORRECT (now)
const API_BASE_URL = 'http://api.jandjessential.org/api/v1  ';
// All APIs now call: http://api.jandjessential.org/api/v1  /...
```
...
### ✅ **API Communication**:
- All calls go to `http://api.jandjessential.org/api/v1  ` instead of Vite dev server
