...
export default function Settings() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      setSettingsForm({
        ...
        orderAutoConfirm: data.orderAutoConfirm,
      ...
      // If 404, provide default settings
  ...
  const fetchBankAccounts = async () => {
    try {
      ...
      // If 404, the bank accounts endpoint is not implemented yet
      if (err?.statusCode === 404 || err?.response?.status === 404) {
        console.log('Bank accounts endpoint not implemented yet')
        setBankAccounts([]) // Set empty array for now
      } else {
