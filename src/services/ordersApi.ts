...
const ordersApi = {
  ...
  getAll: async (params: OrdersQueryParams = {}): Promise<ApiResponse<Order[]>> => {
    try {
      ...
      console.log('🌍 Orders API: Base URL being used:', import.meta.env.VITE_API_URL || 'http://api.jandjessential.org/api/v1');
      ...
      console.log('🔍 Orders API: Full URL would be:', `http://api.jandjessential.org/api/v1/orders?${new URLSearchParams(params as any).toString()}`);
