...
const ordersApi = {
  ...
  getAll: async (params: OrdersQueryParams = {}): Promise<ApiResponse<Order[]>> => {
    try {
      ...
      console.log('🌍 Orders API: Base URL being used:', import.meta.env.VITE_API_URL || 'https://jj-essencial.afribase.dev');
      ...
      console.log('🔍 Orders API: Full URL would be:', `https://jj-essencial.afribase.dev/orders?${new URLSearchParams(params as any).toString()}`);
